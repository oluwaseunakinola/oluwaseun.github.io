...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (std::numeric_limits<T>::max() - result < increment) {
            std::cout << "Numeric overflow detected!" << std::endl;
            return result;
        }
        result += increment;
    }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (std::numeric_limits<T>::min() + result > decrement) {
            std::cout << "Numeric underflow detected!" << std::endl;
            return result;
        }
        result -= decrement;
    }
...
void test_overflow()
{
    ...
    if (result < start) { overflow = true; } //check for overflow
    std::cout << +result << ",Overflow Status: " << overflow<<std::endl;

    overflow = false; //reset the overflow flag for the next test

    std::cout << "\tAdding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";
    result = add_numbers<T>(start, increment, steps + 1);
    if (result < start) { overflow = true; } //check for overflow again
    std::cout << +result << ", Overflow Status: "<<overflow<<std::endl;
}
...
void test_underflow()
{
    ...
    // END DO NOT CHANGE
    bool underflow = false; // set a flag to track underflows.

    std::cout << "\tSubtracting Numbers Without Overflow (" << +start << ", " << +decrement << ", " << steps << ") = ";
    auto result = subtract_numbers<T>(start, decrement, steps);
    if (result > start) { underflow = true; } //check for underflows.
    std::cout << +result <<", UnderFlow Status: "<<underflow<<std::endl;

    underflow = false; //reset the flag to track underflows.
